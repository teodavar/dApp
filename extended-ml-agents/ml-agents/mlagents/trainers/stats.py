...
class ConsoleWriter(StatsWriter):
    ...
    def write_stats(
        ...
        if "Environment/Cumulative Reward" in values:
            ...
            # Teo Thesis
            # Inform trainee about the reward_gained so far
            logger.info("-------> Inform trainee about the reward_gained so far !!!")
            
            message = {
                'reward' : stats_summary.mean,
            }
            r = requests.post(trainee_url + "reward", data=message)
            # end Teo Thesis
