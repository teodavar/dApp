...
def run_training(run_seed: int, options: RunOptions, mydemopath: str) -> None: # Teo mydemopath added
    """
    ...
    with hierarchical_timer("run_training.setup"):
        ...
        print("------> trainer_config:",options.trainer_config )

    # Begin training
    try:
        tc.start_learning(env_manager, mydemopath) # Teo mydemopath added
    finally:
        env_manager.close()
...
def run_cli(options: RunOptions, mydemopath: str) -> None: # Teo mydemopath added
    try:
        print('---------------------------------------------------------------')
        print(
            """
    ...
    run_training(run_seed, options, mydemopath) # Teo mydemopath added


# Teo Thesis
@app.route('/training', methods = ['POST'])
def training():
    mydemopath = request.form['demofile']
    print("----------> Start training with demofile: ", mydemopath)
    runOptions = parse_command_line()
    t = threading.Thread(target=run_cli,args=(runOptions,mydemopath,))
    t.start()
    print("----------------------------------------------------------------------")
...
def main():
    # Teo Thesis
    print("----------> Ready to start the training!!!")
    
    app.run(host='127.0.0.1', port=5100)
    #run_cli(parse_command_line()) # commented inserted by Teo
    # end Teo Thesis


# For python debugger to directly run this script
if __name__ == "__main__":
    main() # commented inserted by Teo
